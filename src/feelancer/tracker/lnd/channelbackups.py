...
class LNDChannelBackupTracker(LndBaseTracker):
    ...
    def _process_item_stream(
        self, item: ln.ChanBackupSnapshot, recon_running: bool
    ) -> Generator[UntransformedData]:

        return self._process_channel_backup(item, recon_running)

    def _new_recon_source(self) -> None:
        return None

    def _get_new_stream(self) -> Callable[[], Generator[UntransformedData]]:
        dispatcher = self._lnd.channel_backup_dispatcher
    ...
    def _process_channel_backup(
        ...
        yield UntransformedData(
            ...
            stream_type=UntransformedStreamType.CHANNEL_BACKUP,
            data=data,
