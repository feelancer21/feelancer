...
from .enums import PolicyFetchType
...
class DBLnChannelStatic(Base):
    """
    All channel data changing over time, like balances and feerates
    """
...
class DBLnChannelPolicy(Base):
    ...
    fetch_type: Mapped[PolicyFetchType] = mapped_column(
        Enum(PolicyFetchType), primary_key=True
    )
    local: Mapped[bool] = mapped_column(Boolean, primary_key=True)
    feerate_ppm: Mapped[int] = mapped_column(BigInteger, nullable=False)
    basefee_msat: Mapped[int] = mapped_column(BigInteger, nullable=False)
    timelockdelta: Mapped[int] = mapped_column(SMALLINT, nullable=False)
